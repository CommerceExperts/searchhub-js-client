...
export const getMapping = (userQuery: string, {tenant}: RequestOptions = {}): Promise<MappedResult> => {
    ...
    const base64Credentials = btoa(conf.customer + ":" + conf.apiKey);
...
export const getSuggestions = (userQuery: string, {tenant}: RequestOptions = {}) => {
    ...
    return fetch(`https://saas.searchhub.io/smartsuggest/v3/${conf.customer}/${conf.channel}?userQuery=${userQuery}`, {
        method: "GET"
    })
        .then(res => res.json());
}
